...
# git add -A
# git commit -m "Submit my project 5: traffic"
# git push origin main:ai50/projects/2020/x/traffic
#
##
...
def load_data(data_dir):
    ...
    for num in range(NUM_CATEGORIES):
        pix = os.listdir(os.path.join(data_dir + os.sep + str(num)))
        names.extend(pix)
        labels.extend([num]*len(pix))
    
    count = 0
    for i in range(len(names)):
        img = cv2.imread(os.path.join(data_dir + os.sep + str(labels[i]) + os.sep + names[i]), cv2.IMREAD_COLOR)
        res = cv2.resize(img, (IMG_WIDTH, IMG_HEIGHT), interpolation = cv2.INTER_AREA)
        images.append(np.array(res))
        count += 1
        print(f"Progress: {count}/{len(names)} images processed")
...
def get_model():
    ...
    model.add(tf.keras.layers.Conv2D(32, (4, 4), strides=(2, 2), activation="relu"))
